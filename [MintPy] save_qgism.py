...
EXAMPLE = """example:
  # save_qgism.py timeseries_ERA5_ramp_demErr.h5 -g inputs/geometrygeo.h5
  save_qgism.py timeseries_ERA5_ramp_demErr.h5 -g inputs/geometryRadar.h5
  save_qgism.py geo/geo_timeseries_ERA5_ramp_demErr.h5 -g geo/geo_geometryRadar.h5
  save_qgism.py timeseries_ERA5_ramp_demErr.h5 -g inputs/geometryRadar.h5 -b 200 150 400 350
"""
...
def write_shape_file(fDict, shp_file, box=None):
    ...
    #Add latitude for each point ############################ MELON ############
    ...
    #Add incidence angle for each point ############################ MELON ############
    fd =  ogr.FieldDefn('INC_ANGLE', ogr.OFTReal)
    fd.SetWidth(7)
    fd.SetPrecision(3)
    layer.CreateField(fd)

    #Add velocity in vertical for each point ############################ MELON ############
            ...
            with h5py.File(fDict['Velocity'], 'r') as velid:
                with h5py.File(fDict['Geometry'], 'r') as geomid:
                    ...
                    for i in range(length):
                        ...
                        inc_angle = geomid['incidenceAngle'][line, box[0]:box[2]].astype(np.float64)            ####### Melon #######
                        ...
                        for j in range(width):
                            ...
                            for ind, date in enumerate(ts_obj.dateList):
                                rdict['D{0}'.format(date)] = ts[ind, j] * 1000 / np.cos(inc_angle[j] * np.pi / 180)        ####### Melon #######
