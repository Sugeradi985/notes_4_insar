...
# Modified By Mingliang Gao, Jun 2021                      #
############################################################
...
EXAMPLE = """example:
  save_qgis_hyp3.py timeseries_ERA5_ramp_demErr.h5 -g inputs/geometryGeo.h5
  save_qgis_hyp3.py timeseries_ERA5_ramp_demErr.h5 -g inputs/geometryRadar.h5
  save_qgis_hyp3.py geo/geo_timeseries_ERA5_ramp_demErr.h5 -g geo/geo_geometryRadar.h5
  save_qgis_hyp3.py timeseries_ERA5_ramp_demErr.h5 -g inputs/geometryRadar.h5 -b 200 150 400 350
"""
...
def gather_files(ts_file, geom_file):
	...
	###### incidence angle ############################################## MELON ############
        inc_file = 'geo_incidenceAngle.h5'
    else:
        vel_file = 'velocity.h5'
        coh_file = 'temporalCoherence.h5'
        msk_file = 'maskTempCoh.h5'
	###### incidence angle ############################################### MELON ############
        inc_file = 'inputs/geometryGeo.h5'
	...
	###### incidence angle ############################################### MELON ############
              'IncidenceAngle': os.path.join(ts_dir, inc_file),
              'Geometry'   : geom_file,
            }
...
def write_shape_file(fDict, shp_file, box=None):
    ...
    #Add latitude for each point ###### incidence angle ###################### MELON ############
    fd = ogr.FieldDefn('LAT', ogr.OFTReal)
    fd.SetWidth(12)
    fd.SetPrecision(3)
    layer.CreateField(fd)

    #Add longitude for each point ############################################ MELON ############
    ...
    #Incidence Angle ###### incidence angle ################################# MELON ############
    fd = ogr.FieldDefn('INC_ANGLE', ogr.OFTReal)
    fd.SetWidth(6)
    fd.SetPrecision(3)
    layer.CreateField(fd)
    
    #Incidence Angle ###### incidence angle ################################# MELON ############
    fd = ogr.FieldDefn('VEL_VTCL', ogr.OFTReal)
    fd.SetWidth(8)
    fd.SetPrecision(2)
    layer.CreateField(fd)
    ...
    with h5py.File(fDict['TimeSeries'], 'r') as tsid:
        with h5py.File(fDict['Coherence'], 'r') as cohid:
            with h5py.File(fDict['Velocity'], 'r') as velid:
                with h5py.File(fDict['IncidenceAngle'], 'r') as incid:   ####### Melon #######
                    with h5py.File(fDict['Geometry'], 'r') as geomid:
                        ...
                        for i in range(length):
                            ...
                            inc_angle = incid['incidenceAngle'][line,:].astype(np.float64)           ####### Melon #######
                            ...
                            for j in range(width):
                                ...
                                rdict = { 'CODE'      : hex(counter)[2:].zfill(8),
                                          ...
                                          'COHERENCE' : coh[j],
                                          'INC_ANGLE' : inc_angle[j],            ####### Melon #######
                                          'VEL_VTCL'  : vel[j]*1000 / np.cos(inc_angle[j] * np.pi / 180),        ####### Melon ####### 
                                          'EFF_AREA'  : 1}
...
def main(iargs=None):
    '''Main driver
    '''
    #incidence_angle.py timeseries.h5 -d inputs/geometryRadar.h5    ###### Melon #######
    #Parse command line
